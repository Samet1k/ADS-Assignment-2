...
public class MyLinkedList<T> implements MyList<T> {
    ...
    public T get(int index) {
        if (index < 0 || index >= size) {  // Индекс теріс немесе тізімнің шегінен асып кетсе
            throw new IndexOutOfBoundsException("Index out of bounds");
        }

        MyNode<T> current = head;
        for (int i = 0; i < index; i++) {
            current = current.next;  // Тізім бойынша алға жылжу
        }

        if (current == null) {  // Егер индекс шегінен асып кетсе (тізімде жоқ болса)
            throw new IndexOutOfBoundsException("Index out of bounds");
        }

        return current.data;  // Нақты деректі қайтарамыз
    }
